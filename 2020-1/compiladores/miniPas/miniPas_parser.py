...
def p_simple_expression (p):
    'simple_expression : sign term   simple_expression2'
...
def p_error(p):
	if VERBOSE:
		if p is not None:
			print ("ERROR SINTACTICO -> " + str(p.lexer.lineno) + " NO SE ESPERABA EL Token  " + str(p.value))
		else:
			    print ("ERROR SINTACTICO -> " + str(cminus_lexer.lexer.lineno))
	else:
		raise Exception('synt', 'error')
"""    
